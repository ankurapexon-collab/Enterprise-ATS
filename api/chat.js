// api/chat.js
// Vercel Serverless Function using Google AI Studio (Free Tier)
...
export default async function handler(req, res) {
  ...
  for (const modelName of MODEL_CANDIDATES) {
    try {
      const { ok, status, data } = await callGemini(modelName, apiKey, system, prompt);
      ...
      res.status(status).json({ 
        error: data?.error?.message || `Gemini request failed with status ${status}` 
      });
      return;
